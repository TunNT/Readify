  ...
  try {
    ...
    return <div className={styles.page}>
      ...
      <main className={styles.container}>
        ...
        <section className={styles.chaptersContainer}><div className={styles.chaptersHeader}><h2 className={styles.sectionTitle}>Chapters ({novel.chapterCount})</h2><div className={styles.chaptersFilter}><Link className={`${styles.filterButton} ${order === "asc" ? styles.active : ""}`} href={`/novels/${slug}?order=asc`}>All</Link><button className={styles.filterButton} type="button">Free</button><Link className={`${styles.filterButton} ${order === "desc" ? styles.active : ""}`} href={`/novels/${slug}?order=desc`}>Latest</Link><Link className={styles.filterButton} href={`/novels/${slug}?order=${order === "asc" ? "desc" : "asc"}`}><ListFilter size={14} /> Sort</Link></div></div><ul className={styles.chaptersList}>{chapters.data.map((chapter) => <li className={styles.chapterItem} key={chapter.slug}><a className={styles.chapterLink} href={`/novels/${slug}/${chapter.slug}`}><span className={styles.chapterTitle}>{chapter.title}</span><time className={styles.chapterDate}>{chapter.publishedAt?.slice(0,10) ?? "2026-06-05"}</time></a></li>)}</ul></section>
      </main>
