...
export function HomeFooter() {
  return <footer className={styles.footer}><div className={styles.footerContent}><div className={styles.footerAbout}><div className={styles.footerLogo}>WeAreNovelArk</div><p>WeAreNovelArk offers a curated selection of free novels for women. Discover romance, fantasy, werewolf, and more — read anywhere, anytime, completely free.</p></div>{groups.map((group) => <div className={styles.footerLinks} key={group.title}><h3>{group.title}</h3><ul>{group.links.map(([label, href]) => <li key={label}>{href === "#" ? <a href="#">{label}</a> : <Link href={href}>{label}</Link>}</li>)}</ul></div>)}</div><div className={styles.copyright}><p>© 2025 WeAreNovelArk. All rights reserved.</p></div></footer>;
}
